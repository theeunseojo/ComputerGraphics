...
// lab2-3
	...
	mat4 trans = T(0,0,-2.5f);
	mat4 rot = Ry(t);
	mat4 total = trans * rot;
	glUniformMatrix4fv(glGetUniformLocation(program,"transformMat"),1,GL_TRUE,total.m);

// lab2-4
	vec3 p = SetVector(0.0f,0.0f,2.0f); //camera position
	vec3 l = SetVector(0,0,0);			// camera right position
	vec3 v = SetVector(0,1,0);			// camera up 

	mat4 lookAtMat = lookAtv(p,l,v);
